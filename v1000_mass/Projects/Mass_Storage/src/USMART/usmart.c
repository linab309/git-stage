...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			v1000_debug("\r\n");
#if USMART_USE_HELP
			v1000_debug("------------------------USMART V2.8------------------------ \r\n");
			v1000_debug("USMART有6个系统命令:\r\n");
			v1000_debug("?:    获取帮助信息\r\n");
			v1000_debug("help: 获取帮助信息\r\n");
			v1000_debug("list: 可用的函数列表\r\n\n");
			v1000_debug("id:   可用函数的ID列表\r\n\n");
			v1000_debug("hex:  参数16进制显示,后跟空格+数字即执行进制转换\r\n\n");
			v1000_debug("dec:  参数10进制显示,后跟空格+数字即执行进制转换\r\n\n");
			v1000_debug("请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");    
			v1000_debug("--------------------------ALIENTEK------------------------- \r\n");
#else
			v1000_debug("指令失效\r\n");
#endif
			break;
		case 2://查询指令
			v1000_debug("\r\n");
			v1000_debug("-------------------------函数清单--------------------------- \r\n");
			for(i=0;i<usmart_dev.fnum;i++)v1000_debug("%s\r\n",usmart_dev.funs[i].name);
			v1000_debug("\r\n");
			break;	 
		case 3://查询ID
			v1000_debug("\r\n");
		...
		case 4://hex指令
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						  	//进制转换功能
				{
					v1000_debug("HEX:0X%X\r\n",res);	   	//转为16进制
				}else if(i!=4)return USMART_PARMERR;//参数错误.
				else 				   				//参数显示设定功能
				{
					v1000_debug("16进制参数显示!\r\n");
		...
		case 6://read指令
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   		//读取指定地址数据功能
				{
					if(usmart_dev.sptype==SP_TYPE_HEX)v1000_debug("*(0X%X)=0X%X\r\n",res,*(u32*)res);	//读取指定地址的值
					else v1000_debug("*(%d)=%d\r\n",res,*(u32*)res);	//读取指定地址的值
...
void usmart_exe(void)
{
	...
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		{
			v1000_debug("%c",'"');			 
			v1000_debug("%s",usmart_dev.parm+usmart_get_parmpos(i));
			v1000_debug("%c",'"');
			temp[i]=(u32)&(usmart_dev.parm[usmart_get_parmpos(i)]);
		}else						  //参数是数字
		{
			temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
			if(usmart_dev.sptype==SP_TYPE_DEC)v1000_debug("%lu",temp[i]);//10进制参数显示
			else v1000_debug("0X%X",temp[i]);//16进制参数显示 	   
		}
		if(i!=pnum-1)v1000_debug(",");
	}
	v1000_debug(") ");
	v1000_debug("\r\n");
	...
	if(rval==1)//需要返回值.
	{
		if(usmart_dev.sptype==SP_TYPE_DEC)v1000_debug("=%lu;\r\n",res);//输出执行结果(10进制参数显示)
		else v1000_debug("=0X%X;\r\n",res);//输出执行结果(16进制参数显示)	   
	}else v1000_debug(";\r\n");//不需要返回值,直接输出结束
