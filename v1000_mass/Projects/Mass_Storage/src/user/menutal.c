...
uc8 menu7[]  ={"7.ùù?ùù"};
uc8 menu8[]  ={"8.ùùùÚ?ù"};
uc8 menu8_a[]  ={"1,.ùùùùù?ùùù?"};
uc8 menu8_b[] ={"2,ùùù??"};
uc8 menu8_c[] ={"3,ù∂?ù?"};
uc8 menu8_d[] ={"4,ùùºùùù?"};
uc8 menu8_e[] ={"5,ù?ùùùùù??ùù"};
uc8 menu8_f[] ={"6,ùùùù??ùùùù"};
uc8 menu8_g[] ={"7,?ùùùùùù"};
#endif
...
typedef struct
{
	u32 latitude;				//?ùù ùùùùùù100000ùù,?ùù?ùùùù100000
	u8 nshemi;					//ùù?/ùù?,N:ùù?;S:ùù?				  
	u32 longitude;			    //ùùùù ùùùùùù100000ùù,?ùù?ùùùù100000
	u8 ewhemi;					//ùùùù/ùùùù,E:ùùùù;W:ùùùù
}HomeGpsStruct;
...
__align(4) u8 dtbuf[50];   								//ùù?ùùùùùù


u8 rtc_set_flag= 1;
//extern u8 ge,shi,bai,qian,wan,shiwan;           //ùù?ùùùù
extern u32 average_speed;
extern int16_t  temperature;
extern long  pressure;
extern long Altitude;											//ùùù??ù
u8 upload = 0;
u8 enter_perss= 0;
float time_zone = 8;
...
typedef struct
{
     u8 MenuID;      //ùùù?ù??ùùùùùù
     u8 ParMenuID;    //ùùù?ù??ùùùùùù
     u8 *MenuName;  //ù?ùù?ùùùùù
     void (*CurrentOperate)(void); //ùùù?ùù?ùù?ùùù
     u8 subnumber;      //ùùù?ù??ùùùùùù
}KeyTabStruct;
//ù?ùùùùù
...
const KeyTabStruct KeyTab[MAX_KEYTABSTRUCT_NUM] =
 {
// ù?ùID                ùùù?ùID       ù?ùùù        ù?ùùùù?ùùù
...
//        system_flag_table->fre_clust = fre_clust;
    ...
    {      
        if(USB_CONNECT == 1)
			sprintf((char *)dtbuf,"--");		    		//ùıùù?ùù?ùùù
	    else
            sprintf((char *)dtbuf,"%02d",system_flag_table->baifenbi);		    		//ùıùù?ùù?ùùù
...
void convert_show_en(float tp,u8 flag,u8 y,u8 mode)
{
    ...
    if(system_flag_table->myunit.COORD == 0)
  	{
		...
		{
    		if(miao<100)
    			sprintf((char *)dtbuf,"%1c  %.6f",flag,miao);	//ùıùùùùùù?ùùù
    		else
    			sprintf((char *)dtbuf,"%1c %.6f",flag,miao);	//ùıùùùùùù?ùùù
		}
	    else
    ...
    else if(system_flag_table->myunit.COORD == 1)
    {
        ...
        if(mode == 1 )
        {
            if(du >100)
            {
                sprintf((char *)dtbuf,"%1c %03d",flag,du);	//ùıùùùùùù?ùùù
            }
            else
            {
                sprintf((char *)dtbuf,"%1c  %02d",flag,du);	//ùıùùùùùù?ùùù
         ...
         if(miao<10)
            sprintf((char *)dtbuf,"0%.3f",miao);	//ùıùùùùùù?ùùù
         else
            sprintf((char *)dtbuf,"%.3f",miao);	//ùıùùùùùù?ùùù
         index += 8;
	...
	else if(system_flag_table->myunit.COORD == 2)
    {
        ...
        Movie_Show_Img(index,y,bmpdu);
        index += 8;
        sprintf((char *)dtbuf,"%02d",fen);	//ùıùùùùùù?ùùù
        OLED_ShowString(index,y,dtbuf);
        index +=16;
        Movie_Show_Img(index,y,bmpfen);
        index += 8;
        if(miao<10.0)
            sprintf((char *)dtbuf,"0%.1f",miao);	//ùıùùùùùù?ùùù
        else
            sprintf((char *)dtbuf,"%.1f",miao);	//ùıùùùùùù?ùùù
        OLED_ShowString(index,y,dtbuf);
        index =120;
...
void convert_show_en_input(u8 *prt_c ,u8 y)
{
	...
	sprintf((char *)dtbuf,"%1c%c%c%c",prt_c[0],prt_c[1],prt_c[2],prt_c[3]);	//ùıùùùùùù?ùùù
	index = 56;
	...
	Movie_Show_Img(index,y,bmpdu);
	index += 8;
	sprintf((char *)dtbuf,"%c%c",prt_c[4],prt_c[5]);	//ùıùùùùùù?ùùù
	OLED_ShowString(index,y,dtbuf);
	index +=16;
	Movie_Show_Img(index,y,bmpfen);
	index += 8;
	sprintf((char *)dtbuf,"%c%c.%c",prt_c[6],prt_c[7],prt_c[8]);	//ùıùùùùùù?ùùù
	OLED_ShowString(index,y,dtbuf);
	index =120;
	Movie_Show_Img(index,y,bmpmaio);
}
void menu_real_crood_display(void)
{
    ...
    tp =gpsx->pdop;
    tp =tp/100;
    sprintf((char *)dtbuf,"%0.1f",tp);	//ùù?UTCùùùù
...
void menu_status_dispplay(void)
{
    ...
    if((system_power_flag == SYSTEM_POWER_LOW)&&(system_flag_table->run_mode == 0))
	{
	    ...
	    if(system_flag_table->myunit.TIMER == 0)
 		{
			    ...
			    if(gpsx_utc/60>=10)
			 	{
				    Movie_Show_Img(1,19,Bmp_bigpm_1);
			 	    sprintf((char *)dtbuf,"%d:%02d:%02d",gpsx_utc/60-10,gpsx_utc%60,RTC_TimeStructure.RTC_Seconds);	//ùù?UTCùùùù
			 	}
			    else
			 	{

			 	    Movie_Show_Img(1,19,Bmp_bigpm);
		 		    sprintf((char *)dtbuf,"%d:%02d:%02d",gpsx_utc/60,gpsx_utc%60,RTC_TimeStructure.RTC_Seconds);	//ùù?UTCùùùù
		    ...
		    else
            {
                ...
                {
                    sprintf((char *)dtbuf,"%d:%02d:%02d",2,gpsx_utc%60,RTC_TimeStructure.RTC_Seconds);	//ùù?UTCùùùù
                    Movie_Show_Img(1,19,Bmp_bigam_1);
                }
                else
                {
		...
		{
            sprintf((char *)dtbuf,"%02d:%02d:%02d",RTC_TimeStructure.RTC_Hours,RTC_TimeStructure.RTC_Minutes,RTC_TimeStructure.RTC_Seconds);	//ùù?UTCùùùù
    ...
    else
	{
	    OLED_Fill(0, 0, 127, 16, 0);
		{
		    if(system_flag_table->myunit.TIMER == 0)
	 		{
	 		    if(gpsx_utc>=12*60)
	 			{
				    ...
				    if(gpsx_utc>=13*60)
			 	    	gpsx_utc= gpsx_utc - 12*60;
		 		    sprintf((char *)dtbuf,"%02d:%02d",gpsx_utc/60,gpsx_utc%60);	//ùù?UTCùùùù
			    ...
			    else
			 	{
			        ...
			        if(gpsx_utc>60)
					    ...
					    sprintf((char *)dtbuf,"%02d:%02d",12,gpsx_utc%60);	//ùù?UTCùùùù
				     OLED_ShowString_head(0,0,dtbuf);
			...
			{
			    sprintf((char *)dtbuf,"%02d:%02d",RTC_TimeStructure.RTC_Hours,RTC_TimeStructure.RTC_Minutes);	//ùù?UTCùùùù
			    OLED_ShowString_head(0,0,dtbuf);
...
void menu_main_display(void)
{
    ...
    {
		case 0:
		 	sprintf((char *)dtbuf,"%02d-%02d-%02d",year,month,data);	//ùù?UTCùùùù
		 	break;
		case 1:
		 	sprintf((char *)dtbuf,"%02d-%02d-%02d",month,data,year);	//ùù?UTCùùùù
		 	break;
		case 2:
		 	sprintf((char *)dtbuf,"%02d-%02d-%02d",data,month,year);	//ùù?UTCùùùù
		 	break;
	...
	{
		sprintf((char *)dtbuf,"%02d:%02d:%02d",hour,min,sec);	//ùù?UTC?ùù
...
//		time_zone = comp_time;
		sprintf((char *)dtbuf,"+%02d:%02d",comp_time,(gpsx_utc-rtc_timer-(comp_time*60)));	//ùù?UTC?ùù
		}
	else
...
//		time_zone = comp_time;
		sprintf((char *)dtbuf,"+%02d:%02d",comp_time,(rtc_timer-gpsx_utc-(comp_time*60)));	//ùù?UTC?ùù
		}
#endif
	...
	if(system_flag_table->myunit.TEMP == 0)
	{
        ...
        sprintf((char *)dtbuf,"%.1f C",temperature_F/10);	//ùıùùùùùù?ùùù

	}
	else if(system_flag_table->myunit.TEMP == 1)
	{
        temperature_F = 320 + (1.8*temperature);
        sprintf((char *)dtbuf,"%.1f F",temperature_F/10);	//ùıùùùùùù?ùùù
    ...
    {
	    sprintf((char *)&dtbuf,"%dm",Altitude/10);	//ùıùùùùùù?ùùù
    }
    else  if(system_flag_table->myunit.H_W >= 1)
    {
		...
		sprintf((char *)&dtbuf,"%dft",feet);	//ùıùùùùùù?ùùù
    }
...
void menu_real_speed_display(void)
{
	...
	if(system_flag_table->myunit.H_W == 0)//||(system_flag_table->myunit.H_W == 1))
	 	sprintf((char *)dtbuf,"%.1fkm/h",tp/1000);		    		//ùıùù?ùù?ùùù

#if 1
	else if(system_flag_table->myunit.H_W == 1)
	 	sprintf((char *)dtbuf,"%.1fmi/h",tp/1609);		    		//ùıùù?ùù?ùùù
#endif
	else if(system_flag_table->myunit.H_W == 2)
	 	sprintf((char *)dtbuf,"%.1fkt/h",tp/1852);		    		//ùıùù?ùù?ùùù
    ...
    else  if(system_flag_table->myunit.H_W >= 1)
    {
        ...
        sprintf((char *)&dtbuf,"%dft",tmp_Att);	//ùıùùùùùù?ùùù
    }
	...
	if(tp <10)
		sprintf((char *)dtbuf,"   %d",(u16)tp);		    		//ùıùù?ùù?ùùù
	else if(tp <100)
		sprintf((char *)dtbuf,"  %d",(u16)tp);		    		//ùıùù?ùù?ùùù
	else
		sprintf((char *)dtbuf," %d",(u16)tp);		    		//ùıùù?ùù?ùùù
...
void menu_temp_change_display(void)
{
	...
	ùùùùùù??ùùùù?ù∂ù=ùùùùùùùùùù∂ù? - |ùùùù∂ù?*0.1|
	GPS??ùùùù?ù∂ù=ùùùù∂ù? - |ùùùù∂ù?*0.22|
	*/
	if(system_flag_table->myunit.TEMP == 0)
	{
		...
		sprintf((char *)dtbuf,"%d",most_temperature/10);	//ùıùùùùùù?ùùù
		
		OLED_ShowString12(0,16,dtbuf);

		sprintf((char *)dtbuf,"%d",lest_temperature/10);	//ùıùùùùùù?ùùù
	...
	else
	{
		...
		sprintf((char *)dtbuf,"%d",temp/10);	//ùıùùùùùù?ùùù
		OLED_ShowString12(0,16,dtbuf);
...
void menu_Pressure_display(void)
{
	...
	if(system_flag_table->myunit.PREE == 0)
    {
		...
		sprintf((char *)dtbuf,"%.1fhpa",pressure_unit/100);	//ùıùùùùùù?ùùù
		OLED_ShowString(57,0,dtbuf);
	
		sprintf((char *)dtbuf,"%d",(int)(most_pressure/100));	//ùıùùùùùù?ùùù
		OLED_ShowString12(0,16,dtbuf);
	
		sprintf((char *)dtbuf,"%d",(int)(lest_pressure/100));	//ùıùùùùùù?ùùù
		OLED_ShowString12(0,46,dtbuf);
	}
	else if(system_flag_table->myunit.PREE == 1)
	{
	    ...
	    pressure_unit = pressure;
		sprintf((char *)dtbuf,"%.1fmba",pressure_unit/100);	//ùıùùùùùù?ùùù
		OLED_ShowString(57,0,dtbuf);
	
		sprintf((char *)dtbuf,"%d",(int)(most_pressure/100));	//ùıùùùùùù?ùùùù?
		OLED_ShowString12(0,16,dtbuf);
	...
	else if(system_flag_table->myunit.PREE == 2)
	{
		...
		sprintf((char *)dtbuf,"%.1fmmhg",pressure_unit/100);	//ùıùùùùùù?ùùù
		OLED_ShowString(57,0,dtbuf);
		pressure_unit = most_pressure/1.333;
		
		sprintf((char *)dtbuf,"%d",(int)(pressure_unit/100));	//ùıùùùùùù?ùùù
		OLED_ShowString12(0,16,dtbuf);
	...
	else if(system_flag_table->myunit.PREE == 3)
	{
		...
		sprintf((char *)dtbuf,"%.2finhg",pressure_unit/100);	//ùıùùùùùù?ùùù
		OLED_ShowString(57,0,dtbuf);
...
//	const unsigned char Bmp_pree_word[];
    ...
    if(system_flag_table->myunit.H_W == 0)
    {
		sprintf((char *)&dtbuf,"%ldm",Altitude/10);	//ùıùùùùùù?ùùù
		OLED_ShowString(65,0,dtbuf);

		sprintf((char *)dtbuf,"%ld",most_Altitude/10);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,16,dtbuf);

		sprintf((char *)dtbuf,"%ld",lest_Altitude/10);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,46,dtbuf);
	//	v1000_debug("\r\nlest_Altitude= %ld",lest_Altitude);
     }
     else  if(system_flag_table->myunit.H_W >= 1)
     {
		sprintf((char *)&dtbuf,"%.0fft",(Altitude/3.048));	//ùıùùùùùù?ùùù
		OLED_ShowString(65,0,dtbuf);

		sprintf((char *)dtbuf,"%.0f",(most_Altitude/3.048));		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,16,dtbuf);

		sprintf((char *)dtbuf,"%.0f",lest_Altitude/3.048);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,46,dtbuf);
...
void menu_speed_change_display(void)
{
	...
	if(system_flag_table->myunit.H_W == 0)
	{
		sprintf((char *)dtbuf,"%dkm/h",(u32)tp/1000);		    		//ùıùù?ùù?ùùù
		OLED_ShowString(70,0,dtbuf);
		tp=most_speed;
		sprintf((char *)dtbuf,"%.1f",tp/1.000);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,16,dtbuf);
		tp=lest_speed;
		sprintf((char *)dtbuf,"%.1f",tp/1.000);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,46,dtbuf);
	...
	else if(system_flag_table->myunit.H_W == 1)
	{
		...
		sprintf((char *)dtbuf,"%dmi/h",(u32)tp/1609);		    		//ùıùù?ùù?ùùù
		OLED_ShowString(70,0,dtbuf);
		tp=most_speed;
		sprintf((char *)dtbuf,"%.1f",tp/1.609);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,16,dtbuf);
		tp=lest_speed;
		sprintf((char *)dtbuf,"%.1f",tp/1.609);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,46,dtbuf);
	...
	else if(system_flag_table->myunit.H_W == 2)
	{
		...
		sprintf((char *)dtbuf,"%dkt/h",(u32)tp/1852);		    		//ùıùù?ùù?ùùù
		OLED_ShowString(70,0,dtbuf);
		tp=most_speed;
		sprintf((char *)dtbuf,"%.1f",tp/1.852);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,16,dtbuf);
		tp=lest_speed;
		sprintf((char *)dtbuf,"%.1f",tp/1.852);		    		//ùıùù?ùù?ùùù
		OLED_ShowString12(0,46,dtbuf);
...
//    FIL test_fp ;
 //   FRESULT test_fr ;
	...
	if(enter_perss == 0)
	{
    	...
    	else
    	{
    		...
    		if(system_flag_table->myunit.H_W == 0)
    		{
    			if(distance>=10 && distance<100)
    				sprintf((char *)dtbuf,"%.2fkm",distance);		    		//ùıùù?ùù?ùùù
    			else if (distance<10)
    				sprintf((char *)dtbuf,"%.3fkm",distance);		    		//ùıùù?ùù?ùùù
    			else
    				sprintf((char *)dtbuf,"%ldkm",(u32)(distance));		    		//ùıùù?ùù?ùùù
    		}
    		else if(system_flag_table->myunit.H_W == 1)
    		{
    			if((distance/1.609)>=10 && (distance/1.609)<100)
    				sprintf((char *)dtbuf,"%.2fmi",distance/1.609);		    		//ùıùù?ùù?ùùù
    			else if ((distance/1.609)<10)
    				sprintf((char *)dtbuf,"%.3fmi",distance/1.609);		    		//ùıùù?ùù?ùùù
    			else
    				sprintf((char *)dtbuf,"%ldmi",(u32)(distance/1.609));		    		//ùıùù?ùù?ùùù
    		...
    		else if(system_flag_table->myunit.H_W == 2)
    		{
    			if((distance/1.852)>=10 && (distance/1.852)<100)
    				sprintf((char *)dtbuf,"%.2fkt",distance/1.852);		    		//ùıùù?ùù?ùùù
    			else if ((distance/1.852)<10)
    				sprintf((char *)dtbuf,"%.3fkt",distance/1.852);		    		//ùıùù?ùù?ùùù
    			else
    				sprintf((char *)dtbuf,"%ldkt",(u32)(distance/1.852));		    		//ùıùù?ùù?ùùù
    	...
    	{
    		if(tp/1000>=100.0)
    			sprintf((char *)dtbuf,"%.0fkm/h",tp/=1000);		    		//ùıùù?ùù?ùùù
    		else
    		 	sprintf((char *)dtbuf,"%.1fkm/h",tp/=1000);		    		//ùıùù?ùù?ùùù
    	}
    	...
    	{
    		if(tp/1609>=100.0)
    			sprintf((char *)dtbuf,"%.0fmi/h",tp/=1609);		    		//ùıùù?ùù?ùùù
    		else
    		 	sprintf((char *)dtbuf,"%.1fmi/h",tp/=1609);		    		//ùıùù?ùù?ùùù
    	}
    	...
    	{
    		if(tp/1852>=100.0)
    			sprintf((char *)dtbuf,"%.0fkt/h",tp/=1852);		    		//ùıùù?ùù?ùùù
    		else
    		 	sprintf((char *)dtbuf,"%.1fkt/h",tp/=1852);		    		//ùıùù?ùù?ùùù
    	}
        ...
        sprintf((char *)dtbuf,"%02d:%02d",timer_need/60,timer_need%60);		    		//ùıùù?ùù?ùùù
	      	...
	      	case 0:
                ...
                else
                {
    			    ...
    			    else if(lat1 >0)
    				{
                        ...
                        if(gpsx->fixmode >1)  // GPSùù?ùùùùùùù?ùùùù?
    					    OLED_ShowString(48,16,dtbuf);
    				}
                }
			    break;
...
int test_sunriset(u16 year,u8 month,u8 day,float lon,float lat)
{
	...
	sunriset( year, month, day, lon, lat,
		altit, upper_limb, &sunrise, &sunset );//ùùùùù?ùùùùùùùù?ùùùùù?ù?ùùùùsunriseùùsunsetùù

	hour = sunrise + zone;
	minute = (sunrise - hour + zone)*60;

    if(gpsx->fixmode!=3)
        sprintf((char *)dtbuf,"--:--");	//ùù?UTCùùùù
    else
        sprintf((char *)dtbuf,"%02d:%02d",hour,minute);	//ùù?UTCùùùù
...
void menu_chaoxi_display(void)
{
	...
	sprintf((char *)dtbuf,"%02d-%02d-%02d",RTC_DateStructure.RTC_Year ,my_timer.w_month,my_timer.w_date);	//ùù?UTCùùùù
	OLED_ShowString(0,48,dtbuf);
	sprintf((char *)dtbuf,"  ");	//ùù?UTCùùùù
       ...
       // memcpy(&guji_data.all,buffer+index+4,4);
        ...
        else
        {
            ...
            if(FR_OK  == sys_fr)
            {
                ...
                f_printf(&sys_fp,"%s",(char *)dtbuf);
                v1000_debug("data1:%s",(char *)dtbuf);
...
void Recording_guji(void)
{
	...
	switch(guji_mode )
    {
		...
		case RECORED_START:
            ...
            // v1000_debug("fixmode :%d ùùlatitude %d,longitude %d\r\n",gpsx->fixmode,gpsx->latitude,gpsx->longitude);
			if((gpsx->fixmode >= 2)&&(gpsx->latitude >0)&&(gpsx->longitude>0))
			{
                ...
                if(my_timer.w_year >= 15)
                {
        			...
        			//	sprintf((char *)dtbuf,"%d",interst_pos_number);		    		//ùıùù?ùù?ùùù
		...
		case RECORED_T:
			...
			//	interst_pos_number = 0;
			interst_pos_number++;
    		//sprintf((char *)dtbuf,"%02d",interst_pos_number);		    		//ùıùù?ùù?ùùù
...
//	u32 distance;
    ...
    if((enter_perss != 1)||(guji_mode != RECORED_IDLE))
    {
        ...
        {
            //	sprintf((char *)dtbuf,"%dkm",distance);		    		//ùıùù?ùù?ùùù
            if(system_flag_table->myunit.H_W == 0)
            {
                if(guiji_distance>1.0)
                    sprintf((char *)dtbuf,"%.2fkm",guiji_distance);		    		//ùıùù?ùù?ùùù
                else
            ...
            else if(system_flag_table->myunit.H_W == 1)
            {
                if((guiji_distance/1.609)>1.0)
                    ...
                    sprintf((char *)dtbuf,"%dft",((u32)(guiji_distance*1000/0.3048)));		    		//ùıùù?ùù?ùùù
            //ùıùù?ùù?ùùù
            }
            ...
            else if(system_flag_table->myunit.H_W == 2)
            {
                if((guiji_distance/1.852)>1.0)
                    sprintf((char *)dtbuf,"%.2fkt",guiji_distance/1.852);		    		//ùıùù?ùù?ùùù
                else
                    sprintf((char *)dtbuf,"%dft",((u32)(guiji_distance*1000/0.3048)));		    		//ùıùù?ùù?ùùù
            }
            OLED_ShowString(0,48,dtbuf);
        ...
        if(guji_icon_index == 0)
        {
            ...
            sprintf((char *)dtbuf,"%d",tp);		    		//ùıùù?ùù?ùùù
        ...
        else if((guji_icon_index == 1)||(guji_icon_index == 2))
        {
            ...
            sprintf((char *)dtbuf,"%02d:%02d",h,m);		    		//ùıùù?ùù?ùùù
            OLED_ShowString(40,32,dtbuf);
        }
        else
        {
            sprintf((char *)dtbuf,"%d",Message_head_number);		    		//ùıùù?ùù?ùùù
					    ...
					    if(system_flag_table->myunit.H_W == 0)
						{
						    if(menu_setting_postion_2<=2)
							{
    							...
    							//	system_flag_table->guji_record.by_distance_vaule= arry_bydistance[i];
    								sprintf((char *)dtbuf,"%dm",arry_bydistance[i]);		    		//ùıùù?ùù?ùùù
					    ...
					    else
						{
						    if(menu_setting_postion_2<=2)
							{
						    	for(i =0;i<3;i++)
								{
    								...
    								sprintf((char *)dtbuf,"%dft",arry_bydistance_2[i]);		    		//ùıùù?ùù?ùùù
    								OLED_ShowString12(16,(29+(i*12)),dtbuf);
								}
							}
					    	else
...
//						stm_read_eerpom(MENU_SETTING_GUJI_BY_TIME ,&setting_tp);
						...
						if(menu_setting_postion_2<=2)
						{
							...
							//	system_flag_table->guji_record.by_time_vaule= arry_time[i];
								sprintf((char *)dtbuf,"%ds",arry_time[i]);		    		//ùıùù?ùù?ùùù
								OLED_ShowString12(16,(29+(i*12)),dtbuf);
							}
						}
						else
				    ...
				    case BY_SPEED:
						...
						if(system_flag_table->myunit.H_W == 0)
						{
							sprintf((char *)dtbuf,"%dkm",menu_setting_postion_2);		    		//ùıùù?ùù?ùùù
						}
						else if(system_flag_table->myunit.H_W == 1)
						{
							sprintf((char *)dtbuf,"%dmi",menu_setting_postion_2);		    		//ùıùù?ùù?ùùù
						}
						else
						{
							sprintf((char *)dtbuf,"%dkt",menu_setting_postion_2);		    		//ùıùù?ùù?ùùù
						}
						#endif
