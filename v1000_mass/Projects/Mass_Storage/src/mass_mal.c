...
#else
     //v1000_debug("write Memory_Offset = %x,Transfer_Length = %x \r\n",Memory_Offset,Transfer_Length);
     //SPI_Flash_writePage((uint8_t*)Writebuff, Memory_Offset, Transfer_Length);
...
//|| defined(USE_STM32L152D_EVAL)
     ...
     // v1000_debug("read Memory_Offset = %x,Transfer_Length = %x \r\n",Memory_Offset,Transfer_Length);
...
//  uint32_t DeviceSizeMul = 0;
  ...
  if (lun == 0)
  {
...
#if 0//defined (USE_STM3210E_EVAL)  || defined(USE_STM32L152D_EVAL)
    if (SD_Init() == SD_OK)
    {
    ...
    //emp_block_mul = (1 << SD_csd.RdBlockLen)/ 512;
    Mass_Block_Count[0] =  NAND_BLOCK_SIZE*NAND_ZONE_SIZE ;
    Mass_Block_Size[0] = NAND_PAGE_SIZE;
    Mass_Memory_Size[0] = (Mass_Block_Count[0] * Mass_Block_Size[0]);
#endif /* USE_STM3210E_EVAL */
  //    Mass_Memory_Size[0] = Mass_Block_Count[0] * Mass_Block_Size[0];
//      STM_EVAL_LEDOn(LED2);
    system_flag_table->usbdisk_flag = 1;
    return MAL_OK;
